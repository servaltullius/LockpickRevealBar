...
- 색으로 방향을 추측하지 못하도록 새 자물쇠마다(기본값: 락픽이 부러질 때도) 다음을 무작위로 바꿉니다.
  - 팔레트 7종
  - 색반전: 그라데이션 방향이 뒤집혀 스윗스팟 가까운 곳이 어둡고 먼 곳이 밝아짐
  - 계단화: 그라데이션을 5~10단계로 끊음
  - 칸마다 고정된 노이즈
- 흰 세로선은 현재 락픽 위치입니다.
- 바 아래 얇은 줄은 현재 락픽의 **내구도**입니다. 초록 → 노랑 → 빨강으로 줄어들고, 0이 되면 락픽이 부러집니다.
- 밝혀지는 중인 칸은 회색 안개로 보이고, 다 밝혀져야 그 위치의 색이 나타납니다. 그래서 덜 밝혀진 칸이 "차가운 색"으로 오해되지 않습니다.
- 스윗스팟 칸은 반전 여부와 상관없이 항상 그 팔레트의 가장 밝은 끝 색입니다. 나머지 칸은 노이즈, 계단화, 반전이 적용돼도 그 색에 닿지 않습니다.
...
락픽 내구도(0~100)는 `LockpickingMenu` 멤버가 아니라 전역 float입니다. 락픽이 버티는 시간은 legacy `0xE8`, shifted `0xFC`에 있습니다. 엔진은 이 값을 매 프레임 `-100 / 버티는 시간 × dt`만큼 줄이고, 0~100으로 고정한 뒤 SWF의 `UpdatePickHealth`로 넘깁니다.

- **AE:** Address Library ID `382911`로 찾습니다. 1.6.1170과 1.7.104에서 초기값 100과 주변 전역 변수들이 일치하는 것을 확인했고, 1.6.353부터 1.7.99까지 모든 AE DB에 있습니다.
- **SE:** ID 체계가 달라서, 실행 중에 이 값을 쓰는 코드(저장 → 100으로 고정 → 0으로 고정)를 `.text`에서 패턴으로 찾습니다. 이 패턴은 1.7.104에서 유일하게 한 곳만 맞습니다. AE에서도 이 패턴을 함께 검색해서, ID 결과와 일치하는지 로그에 남깁니다.
